...
def run_predict(config: YOLOPredictConfig):
    ...
    if not config.model_path:
        raise RuntimeError("Model path is required for prediction")

    model_file = Path(config.model_path)
    if not model_file.exists():
        raise RuntimeError(f"Model file not found: {config.model_path}")

    # Validate source
    if not config.source:
        raise RuntimeError("Source (image/video path) is required for prediction")

    source_path = Path(config.source)
    if not source_path.exists():
        raise RuntimeError(f"Source path does not exist: {config.source}")
