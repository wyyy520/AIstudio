...
def run_train(config: YOLOTrainConfig):
    ...
    if not dataset_path.exists():
        raise RuntimeError(f"Dataset path does not exist: {config.dataset}")
    ...
    if not data_yaml.exists():
        raise RuntimeError(f"No data.yaml or dataset.yaml found in {config.dataset}")
    ...
    try:
        ...
        raise RuntimeError(f"ultralytics not installed: {e}. Run: pip install ultralytics")
