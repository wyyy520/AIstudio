...
func TestNewGenerator(t *testing.T) {
	g := NewGenerator()
	if g.ID() != common.Target("ros2") {
		t.Errorf("expected ID ros2, got %s", g.ID())
	}
	if g.Name() == "" {
...
func TestGeneratorRuntimeRequirement(t *testing.T) {
	...
	if rr.Name != "ros" {
		t.Errorf("expected ros, got %s", rr.Name)
	}
...
func TestGenerateBasic(t *testing.T) {
	...
	expectedFiles := []string{
		...
		"setup.cfg",
		".gitignore",
		"launch/test_project_launch.py",
		"resource/test_project",
		"aistudio_ws/__init__.py",
		"aistudio_ws/data_loader_node.py",
		"aistudio_ws/inference_node.py",
		"test/test_nodes.py",
		"test/__init__.py",
	}
	...
	pkgPath := filepath.Join(outputDir, "package.xml")
	pkgData, err := os.ReadFile(pkgPath)
	if err != nil {
		t.Fatalf("failed to read package.xml: %v", err)
	}
	if !strings.Contains(string(pkgData), "test_project") {
		t.Error("package.xml should contain package name")
	}
}
...
func TestNodeTypeToROS2Name(t *testing.T) {
	tests := []struct {
		nodeType string
		expected string
	}{
		...
		{"model_inference", "inference_node"},
		{"custom", "custom_node"},
	}

	for _, tt := range tests {
		result := nodeTypeToROS2Name(tt.nodeType)
		if result != tt.expected {
			t.Errorf("nodeTypeToROS2Name(%q) = %q, want %q", tt.nodeType, result, tt.expected)
		}
	}
}
