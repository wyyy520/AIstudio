    ...
    <div class="log-viewer-content">
      <div v-if="activeTab === 'human'" class="human-log">
        <div
          ...
          :class="[`step-${entry.stepStatus}`, { 'has-error': entry.level === 'error' }]"
        >
          <div class="step-row">
          ...
          <div v-if="entry.level === 'error' && expandedEntries.has(entry.id)" class="step-raw">
            <pre class="raw-content selectable">{{ entry.rawMessage }}</pre>
          </div>
          <button
          ...
          >
            <svg :class="{ expanded: expandedEntries.has(entry.id) }" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
              <path d="m6 9 6 6 6-6" />
            </svg>
            {{ expandedEntries.has(entry.id) ? '收起' : '查看详细' }}
          </button>
        </div>
        ...
        <div v-if="!logs.length" class="log-empty">
          ...
          <span class="log-empty-title">暂无日志</span>
          <span class="log-empty-desc">选择一个任务查看日志</span>
        </div>
      </div>
