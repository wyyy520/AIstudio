    ...
    <div v-if="visible" class="fix-dialog-overlay" @click.self="$emit('cancel')">
      <div class="fix-dialog">
        <div class="fix-dialog-header">
          ...
          <span class="fix-dialog-title">执行修复</span>
        </div>

        <div class="fix-dialog-body">
          <p class="fix-dialog-desc">AI 建议执行以下修复操作:</p>
          ...
          <div v-if="isFixing" class="fix-warning">
            ...
            <span>此操作将修改您的环境配置</span>
          </div>
        </div>

        <div class="fix-dialog-footer">
          <AppButton type="ghost" size="medium" label="取消" :disabled="isFixing" @click="$emit('cancel')" />
          <AppButton
            type="primary"
            size="medium"
            :label="isFixing ? '修复中...' : '确认修复'"
            :loading="isFixing"
            :disabled="isFixing"
            @click="$emit('execute')"
          />
