...
function validateEnvironment(nodeId: string, data: WorkflowNodeData): ValidationError[] {
  ...
  if (data.templateKey === 'cuda_check') {
    // CUDA availability check - will be replaced with real detection
    errors.push({
      type: 'env-warning',
      nodeId,
      message: '请确认 GPU 环境已就绪（CUDA 检测将在运行时自动执行）',
      severity: 'info',
      autoFix: '',
      details: 'CUDA 环境将在工作流运行时由 Engine 自动检测',
    })
  }
  ...
  if (data.templateKey === 'yolo_training' && params.device === 'cuda') {
    errors.push({
      type: 'env-warning',
      nodeId,
      message: `"${data.label}" 配置了 CUDA 模式，运行时将自动检测 GPU 可用性`,
      severity: 'info',
      autoFix: '',
      details: '如果 GPU 不可用，Engine 将自动回退到 CPU 模式',
    })
  }
