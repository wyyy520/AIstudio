...
// ListSkillsTool lists available skills (workflow templates).
type ListSkillsTool struct {
	ListFn func(ctx context.Context) ([]map[string]interface{}, error)
}

func (t *ListSkillsTool) Name() string       { return "list_skills" }
func (t *ListSkillsTool) Description() string { return "List all available skills (workflow templates) and their descriptions." }
func (t *ListSkillsTool) Parameters() []ToolParameter { return nil }

func (t *ListSkillsTool) Execute(ctx context.Context, params map[string]interface{}) (*ToolResult, error) {
	if t.ListFn == nil {
		return &ToolResult{Success: false, Error: "list_skills: not configured"}, nil
	}
	skills, err := t.ListFn(ctx)
	if err != nil {
		return &ToolResult{Success: false, Error: err.Error()}, nil
	}
	return &ToolResult{Success: true, Data: map[string]interface{}{"skills": skills}}, nil
}

// ApplySkillTool applies a skill template to create a workflow.
type ApplySkillTool struct {
	ApplyFn func(ctx context.Context, skillID string, params map[string]interface{}) (string, error)
}

func (t *ApplySkillTool) Name() string       { return "apply_skill" }
func (t *ApplySkillTool) Description() string { return "Apply a skill template to create a workflow with custom parameters." }
func (t *ApplySkillTool) Parameters() []ToolParameter {
	return []ToolParameter{
		{Name: "skill_id", Type: "string", Description: "ID of the skill to apply", Required: true},
		{Name: "parameters", Type: "object", Description: "Skill parameters", Required: false},
	}
}

func (t *ApplySkillTool) Execute(ctx context.Context, params map[string]interface{}) (*ToolResult, error) {
	skillID, _ := params["skill_id"].(string)
	if skillID == "" {
		return &ToolResult{Success: false, Error: "skill_id is required"}, nil
	}
	skillParams, _ := params["parameters"].(map[string]interface{})

	if t.ApplyFn == nil {
		return &ToolResult{Success: false, Error: "apply_skill: not configured"}, nil
	}
	workflowJSON, err := t.ApplyFn(ctx, skillID, skillParams)
	if err != nil {
		return &ToolResult{Success: false, Error: err.Error()}, nil
	}
