...
func TestDetectCUDA(t *testing.T) {
	...
	if info.Version == "" && len(info.GPUs) == 0 {
		t.Log("CUDA/GPU not detected (this may be expected)")
	} else {
		t.Logf("CUDA version: %s", info.Version)
...
`
	...
	for i, pkg := range expected {
		if packages[i].Name != pkg {
			t.Errorf("expected %q at index %d, got %q", pkg, i, packages[i].Name)
		}
	}
}
...
func TestManagerWithMockDetectors(t *testing.T) {
	...
	if status.CUDA.Version != "12.1" {
		t.Errorf("expected CUDA 12.1, got %s", status.CUDA.Version)
	}
...
func TestManagerGetStatus(t *testing.T) {
	...
	result := mgr.GetStatus()
	status, ok := result.(EnvironmentStatus)
	if !ok {
		t.Fatalf("expected EnvironmentStatus, got %T", result)
	}
