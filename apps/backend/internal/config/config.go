...
type EngineConfig struct {
	...
	URL        string        `mapstructure:"url" json:"url"`
	Timeout    time.Duration `mapstructure:"timeout" json:"timeout"` // max execution time (0 = no timeout)
}
...
type LLMConfig struct {
	Provider    string        `mapstructure:"provider" json:"provider"`           // "openai", "claude", "gemini", "qwen", "zhipu", "deepseek", "mock"
...
func setDefaults(v *viper.Viper) {
	...
	v.SetDefault("engine.python_path", "python")
	v.SetDefault("engine.engine_dir", "../Engine")
	v.SetDefault("engine.url", "http://localhost:8082")
	v.SetDefault("engine.timeout", "30m")
...
func loadInternal(explicitPath string) error {
	...
	if env == "production" && cfg.JWT.Secret == "aistudio-default-secret-change-in-production" {
		log.Println("[config] ⚠️  WARNING: Using default JWT secret in production! Set AISTUDIO_JWT_SECRET environment variable or configure jwt.secret in config.")
	}
	return nil
}
