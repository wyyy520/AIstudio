...
func (c *Config) resolvePaths() error {
	...
	if c.Engine.EngineDir != "" && !filepath.IsAbs(c.Engine.EngineDir) {
		c.Engine.EngineDir = filepath.Join(absRoot, c.Engine.EngineDir)
	}
	...
	if c.Engine.VenvPath != "" && !filepath.IsAbs(c.Engine.VenvPath) {
		c.Engine.VenvPath = filepath.Join(absRoot, c.Engine.VenvPath)
	}
	...
	if c.Frontend.ViteDir != "" && !filepath.IsAbs(c.Frontend.ViteDir) {
		c.Frontend.ViteDir = filepath.Join(absRoot, c.Frontend.ViteDir)
	}
...
func CheckDependencies(cfg *Config) error {
	...
	if backendExe == "" {
		backendExe = filepath.Join(cfg.Paths.BackendDir, "cmd.exe")
	}
	if !fileExists(backendExe) {
		errs = append(errs, fmt.Errorf("Backend 可执行文件不存在: %s", backendExe))
	} else {
		logInfo("Backend 可执行文件检查通过", "path", backendExe)
	}
