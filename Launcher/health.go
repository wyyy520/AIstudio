...
func (hm *HealthMonitor) checkBackend(ctx context.Context) {
	serviceName := "backend"
	healthHost := hm.cfg.Backend.Host
	if healthHost == "0.0.0.0" {
		healthHost = "127.0.0.1"
	}
	healthURL := fmt.Sprintf("http://%s:%s%s", healthHost, hm.cfg.Backend.Port, hm.cfg.Backend.HealthPath)
...
func (hm *HealthMonitor) checkEngine(ctx context.Context) {
	serviceName := "engine"
	healthHost := hm.cfg.Engine.Host
	if healthHost == "0.0.0.0" {
		healthHost = "127.0.0.1"
	}
	healthURL := fmt.Sprintf("http://%s:%s%s", healthHost, hm.cfg.Engine.Port, hm.cfg.Engine.HealthPath)
