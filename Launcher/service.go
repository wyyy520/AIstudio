...
func (s *LauncherService) Start(ctx context.Context, noFrontend bool) error {
	...
	backendHealthHost := s.cfg.Backend.Host
	if backendHealthHost == "0.0.0.0" {
		backendHealthHost = "127.0.0.1"
	}
	backendURL := fmt.Sprintf("http://%s:%s%s", backendHealthHost, s.cfg.Backend.Port, s.cfg.Backend.HealthPath)
	...
	engineHealthHost := s.cfg.Engine.Host
	if engineHealthHost == "0.0.0.0" {
		engineHealthHost = "127.0.0.1"
	}
	engineURL := fmt.Sprintf("http://%s:%s%s", engineHealthHost, s.cfg.Engine.Port, s.cfg.Engine.HealthPath)
...
func (s *LauncherService) startBackend(ctx context.Context) error {
	...
	if backendExe == "" {
		backendExe = filepath.Join(s.cfg.Paths.BackendDir, "aistudio-backend.exe")
	}

	// If the compiled binary doesn't exist, try `go run`
	if !fileExists(backendExe) {
		logWarn("Backend 可执行文件不存在，使用 go run 模式", "expected", backendExe)
		goRunCmd := exec.CommandContext(ctx, "go", "run", "./cmd/")
		goRunCmd.Dir = s.cfg.Paths.BackendDir
		goRunCmd.Env = append(os.Environ(),
			fmt.Sprintf("SERVER_HOST=%s", s.cfg.Backend.Host),
			fmt.Sprintf("SERVER_PORT=%s", s.cfg.Backend.Port),
			fmt.Sprintf("AISTUDIO_ENV=%s", s.cfg.App.Environment),
		)
		backendLog := filepath.Join(s.cfg.Paths.LogsDir, "backend.log")
		return s.pm.Start("backend", goRunCmd, backendLog)
	}
...
func (s *LauncherService) startViteDevServer(ctx context.Context) error {
	...
	// 检查 Vite 是否已经在运行
	viteURL := fmt.Sprintf("http://127.0.0.1:%s", vitePort)
